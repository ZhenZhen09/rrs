...
export const CategoryPicker: React.FC<CategoryPickerProps> = ({
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        className="w-[320px] p-0 rounded-3xl border-none shadow-2xl overflow-hidden"
        align="start"
      >
        <Command className="rounded-none border-none">
          <div className="p-4 bg-slate-900">
            <CommandInput
              placeholder="Search 30+ categories..."
              className="h-8 bg-white/10 border-none text-white placeholder:text-slate-400 rounded-xl"
            />
          </div>
          <CommandList className="max-h-[200px]">
            ...
            {Object.entries(REQUEST_CATEGORIES).map(([group, types]) => (
              <CommandGroup
                key={group}
                heading={
                  <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 px-2 py-2">
                    {group}
                  </span>
                }
              >
