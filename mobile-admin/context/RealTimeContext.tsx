...
export interface RiderLocation {
  lat: number;
  lng: number;
  name: number | string; // personnelId/name
  lastUpdate: string;
}
...
export const RealTimeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    socket.on('rider-location-updated', (data: { riderId: string; location: RiderLocation }) => {
      setRiderLocations((prev) => {
        const next = new Map(prev);
        next.set(data.riderId, data.location);
        return next;
      });
    });
  ...
  return (
    <RealTimeContext.Provider value={{ riderLocations, riderPresence, isConnected }}>
      {children}
